...
Sub SetConnection(shost As String, susername As String, spassword As String, sdbname As String) As BANanoMySQL
	host = shost
	username = susername
	password = spassword
	DBase = sdbname
...
'get table structure
Sub GetTableStructure As BANanoMySQL
...
Public Sub Initialize(dbName As String, tblName As String, PK As String) As BANanoMySQL
	Schema.Initialize
	recType.Initialize
	Record.Initialize 
...
Sub SchemaClear As BANanoMySQL
	Schema.clear
	Return Me
End Sub

Sub SetField(fldName As String, fldValue As Object) As BANanoMySQL
	Record.Put(fldName, fldValue)
	Return Me
End Sub

'schema add boolean
Sub SchemaAddBoolean(bools As List) As BANanoMySQL
...
Sub SchemaAddFloat(bools As List) As BANanoMySQL
	For Each b As String In bools
...
Sub SchemaAddBlob(bools As List) As BANanoMySQL
	For Each b As String In bools
...
Sub SchemaAddText(bools As List) As BANanoMySQL
	For Each b As String In bools
...
Sub SchemaCreateTable(tblName As String, PK As String, Auto As String) As BANanoMySQL
	Return CreateTable(tblName, Schema, PK, Auto)
End Sub

Sub SchemaAddDate(bools As List) As BANanoMySQL
	For Each b As String In bools
...
Sub Connection As BANanoMySQL
	command = "connection"
	Return Me
End Sub

' return string to create database
Sub CreateDatabase As BANanoMySQL
...
'drop the database
Sub DropDataBase As BANanoMySQL
...
public Sub CreateTable(tblName As String, tblFields As Map, PK As String, Auto As String) As BANanoMySQL
...
public Sub DropTable As BANanoMySQL
	'define the qry to execute
...
'return a sql insert statement
Sub InsertReplace As BANanoMySQL
	Dim sb As StringBuilder
	Dim columns As StringBuilder
	Dim values As StringBuilder
	Dim listOfValues As List = GetMapValues(Record)
...
Sub Delete(primaryValue As String) As BANanoMySQL
...
'read
Sub Read(primaryValue As String) As BANanoMySQL
	Dim qw As Map = CreateMap()
	qw.Put(PrimaryKey, primaryValue)
...
'exists
Sub Exists(primaryValue As String) As BANanoMySQL
	Dim qw As Map = CreateMap()
	qw.Put(PrimaryKey, primaryValue)
...
Sub SelectWhere(tblName As String, tblfields As List, tblWhere As Map, operators As List, orderBy As List) As BANanoMySQL
	If Schema.Size = 0 Then
...
'return a sql to select record of table where one exists
Sub SelectDistinctWhere(tblName As String, tblfields As List, tblWhere As Map, operators As List, orderBy As List) As BANanoMySQL
	If Schema.Size = 0 Then
		Log($"BANanoMySQL.SelectWhere: '${tblName}' schema is not set!"$)
	End If
	If operators = Null Then operators = EQOperators(tblWhere)
...
'return a sql to delete record of table where one exists
Sub DeleteWhere(tblName As String, tblWhere As Map, operators As List) As BANanoMySQL
	If Schema.Size = 0 Then
...
'return a sql to select record of table where one exists
Sub SelectAll(tblfields As List, orderBy As List) As BANanoMySQL
	'are we selecting all fields or just some
	Dim fld1 As String = tblfields.Get(0)
	Dim selFIelds As String = ""
	Select Case fld1
...
'return a sql to select record of table where one exists
Sub SelectDistinctAll(tblfields As List, orderBy As List) As BANanoMySQL
	'are we selecting all fields or just some
	Dim fld1 As String = tblfields.Get(0)
	Dim selFIelds As String = ""
	Select Case fld1
...
private Sub AddStrings(fieldNames As List) As BANanoMySQL
...
private Sub AddIntegers(fieldNames As List) As BANanoMySQL
	For Each strfld As String In fieldNames
...
private Sub AddDoubles(fieldNames As List) As BANanoMySQL
	For Each strfld As String In fieldNames
...
'specify double field types
private Sub AddBooleans(fieldNames As List) As BANanoMySQL
	For Each strfld As String In fieldNames
...
private Sub AddBlobs(fieldNames As List) As BANanoMySQL
	For Each strfld As String In fieldNames
...
Sub Update(priValue As String) As BANanoMySQL
...
Sub UpdateWhere(tblName As String, tblfields As Map, tblWhere As Map, operators As List) As BANanoMySQL
	If Schema.Size = 0 Then
...
Sub UpdateAll(tblFields As Map, operators As List) As BANanoMySQL
