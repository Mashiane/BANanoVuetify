...
#DesignerProperty: Key: Loading, DisplayName: Loading, FieldType: String, DefaultValue:  , Description: If `true` and no items are provided, then a loading text will be shown
...
If BANano.IsUndefined(eOnCurrentItems) Or BANano.IsNull(eOnCurrentItems) Then eOnCurrentItems = ""
Dim sCode As String = $"${sName}(${eOnCurrentItems})"$
AddAttr("v-on:current-items", sCode)
...
Sub SetOnCurrentItemsE(sCurrentItems As String)
