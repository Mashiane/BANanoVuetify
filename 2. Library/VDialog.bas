...
#DesignerProperty: Key: Transition, DisplayName: Transition, FieldType: String, DefaultValue: dialog-transition , Description: Sets the component transition. Can be one of the built in transitions/styles/transitions or one your own.
...
Private stTransition As String = "dialog-transition"
...
If BANano.IsUndefined(eOnClickOutside) Or BANano.IsNull(eOnClickOutside) Then eOnClickOutside = ""
Dim sCode As String = $"${sName}(${eOnClickOutside})"$
AddAttr("v-on:click:outside", sCode)
...
Sub SetOnClickOutsideE(sClickOutside As String)
