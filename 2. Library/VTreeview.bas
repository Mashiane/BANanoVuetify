...
#DesignerProperty: Key: Activatable, DisplayName: Activatable, FieldType: Boolean, DefaultValue: False , Description: Allows user to mark a node as active by clicking on it
#DesignerProperty: Key: Active, DisplayName: Active, FieldType: String, DefaultValue:  , Description: Syncable prop that allows one to control which nodes are active. The array consists of the `item-key` of each active item.
#DesignerProperty: Key: Color, DisplayName: Color, FieldType: String, DefaultValue: primary , Description: Sets the color of the active node
...
#DesignerProperty: Key: Dense, DisplayName: Dense, FieldType: Boolean, DefaultValue: False , Description: Decreases the height of the items
#DesignerProperty: Key: Filter, DisplayName: Filter, FieldType: String, DefaultValue:  , Description: Custom item filtering function. By default it will use case-insensitive search in items label.
#DesignerProperty: Key: Hoverable, DisplayName: Hoverable, FieldType: Boolean, DefaultValue: False , Description: Applies a hover class when mousing over nodes
#DesignerProperty: Key: Items, DisplayName: Items, FieldType: String, DefaultValue:  , Description: An array of items used to build the treeview
...
#DesignerProperty: Key: Open, DisplayName: Open, FieldType: String, DefaultValue:  , Description: Syncable prop that allows one to control which nodes are open. The array consists of the `item-key` of each open item.
...
#DesignerProperty: Key: Rounded, DisplayName: Rounded, FieldType: Boolean, DefaultValue: False , Description: Provides an alternative active style for `v-treeview` node. Only visible when `activatable` is `true` and should not be used in conjunction with the `shaped` prop.
#DesignerProperty: Key: Search, DisplayName: Search, FieldType: String, DefaultValue:  , Description: The search model for filtering results
#DesignerProperty: Key: Selectable, DisplayName: Selectable, FieldType: Boolean, DefaultValue: False , Description: Will render a checkbox next to each node allowing them to be selected
#DesignerProperty: Key: Shaped, DisplayName: Shaped, FieldType: Boolean, DefaultValue: False , Description: Provides an alternative active style for `v-treeview` node. Only visible when `activatable` is `true` and should not be used in conjunction with the `rounded` prop.
#DesignerProperty: Key: Transition, DisplayName: Transition, FieldType: Boolean, DefaultValue: False , Description: Applies a transition when nodes are opened and closed
...
#DesignerProperty: Key: Value, DisplayName: Value, FieldType: String, DefaultValue:  , Description: Allows one to control which nodes are selected. The array consists of the `item-key` of each selected item. Is used with `@input` event to allow for `v-model` binding.
'***** STYLES *****
...
Private boActivatable As Boolean = False
Private stActive As String = ""
...
AddAttr("activatable", boActivatable)
AddAttr("active", stActive)
AddAttr("color", stColor)
AddAttr("dark", boDark)
AddAttr("dense", boDense)
AddAttr("filter", stFilter)
AddAttr("hoverable", boHoverable)
...
AddAttr("selectable", boSelectable)
AddAttr("shaped", boShaped)
AddAttr("transition", boTransition)
...
'set Allows user to mark a node as active by clicking on it
public Sub setActivatable(varActivatable As Boolean)
AddAttr("activatable", varActivatable)
boActivatable = varActivatable
End Sub

'get Allows user to mark a node as active by clicking on it
public Sub getActivatable() As Boolean
Return boActivatable
End Sub

'set Syncable prop that allows one to control which nodes are active. The array consists of the `item-key` of each active item.
public Sub setActive(varActive As String)
AddAttr("active", varActive)
stActive = varActive
End Sub

'get Syncable prop that allows one to control which nodes are active. The array consists of the `item-key` of each active item.
public Sub getActive() As String
Return stActive
End Sub

'set Sets the color of the active node
public Sub setColor(varColor As String)
AddAttr("color", varColor)
stColor = varColor
End Sub

'get Sets the color of the active node
...
'set Decreases the height of the items
public Sub setDense(varDense As Boolean)
AddAttr("dense", varDense)
boDense = varDense
End Sub

'get Decreases the height of the items
public Sub getDense() As Boolean
Return boDense
End Sub

'set Custom item filtering function. By default it will use case-insensitive search in items label.
public Sub setFilter(varFilter As String)
AddAttr("filter", varFilter)
stFilter = varFilter
End Sub

'get Custom item filtering function. By default it will use case-insensitive search in items label.
public Sub getFilter() As String
Return stFilter
End Sub

'set Applies a hover class when mousing over nodes
public Sub setHoverable(varHoverable As Boolean)
AddAttr("hoverable", varHoverable)
boHoverable = varHoverable
End Sub

'get Applies a hover class when mousing over nodes
public Sub getHoverable() As Boolean
Return boHoverable
End Sub

'set An array of items used to build the treeview
public Sub setItems(varItems As String)
AddAttr("items", varItems)
stItems = varItems
End Sub

'get An array of items used to build the treeview
...
'set Syncable prop that allows one to control which nodes are open. The array consists of the `item-key` of each open item.
public Sub setOpen(varOpen As String)
AddAttr("open", varOpen)
stOpen = varOpen
End Sub

'get Syncable prop that allows one to control which nodes are open. The array consists of the `item-key` of each open item.
public Sub getOpen() As String
Return stOpen
End Sub
...
'set Provides an alternative active style for `v-treeview` node. Only visible when `activatable` is `true` and should not be used in conjunction with the `shaped` prop.
public Sub setRounded(varRounded As Boolean)
AddAttr("rounded", varRounded)
boRounded = varRounded
End Sub

'get Provides an alternative active style for `v-treeview` node. Only visible when `activatable` is `true` and should not be used in conjunction with the `shaped` prop.
public Sub getRounded() As Boolean
Return boRounded
End Sub

'set The search model for filtering results
public Sub setSearch(varSearch As String)
AddAttr("search", varSearch)
stSearch = varSearch
End Sub

'get The search model for filtering results
public Sub getSearch() As String
Return stSearch
End Sub

'set Will render a checkbox next to each node allowing them to be selected
public Sub setSelectable(varSelectable As Boolean)
AddAttr("selectable", varSelectable)
boSelectable = varSelectable
End Sub

'get Will render a checkbox next to each node allowing them to be selected
public Sub getSelectable() As Boolean
Return boSelectable
End Sub

'set Provides an alternative active style for `v-treeview` node. Only visible when `activatable` is `true` and should not be used in conjunction with the `rounded` prop.
public Sub setShaped(varShaped As Boolean)
AddAttr("shaped", varShaped)
boShaped = varShaped
End Sub

'get Provides an alternative active style for `v-treeview` node. Only visible when `activatable` is `true` and should not be used in conjunction with the `rounded` prop.
public Sub getShaped() As Boolean
Return boShaped
End Sub

'set Applies a transition when nodes are opened and closed
public Sub setTransition(varTransition As Boolean)
AddAttr("transition", varTransition)
boTransition = varTransition
End Sub

'get Applies a transition when nodes are opened and closed
public Sub getTransition() As Boolean
Return boTransition
End Sub
...
'set Allows one to control which nodes are selected. The array consists of the `item-key` of each selected item. Is used with `@input` event to allow for `v-model` binding.
public Sub setValue(varValue As String)
AddAttr("value", varValue)
stValue = varValue
End Sub

'get Allows one to control which nodes are selected. The array consists of the `item-key` of each selected item. Is used with `@input` event to allow for `v-model` binding.
