...
Sub Code
	...
	'
	allspent.Initialize(vue, "allspent", Me).SetIcon("attach_money").SetFrom("0").SetTo("3967").SetText("Overall Spent")
	...
	'
	mostspentby.Initialize(vue, "mostspentby", Me).SetIcon("insert_chart").SetFrom("0").SetTo("1800").SetText("Cheque").SetIconBackgroundColor(vm.COLOR_ORANGE)
	mostspentby.SetHoverExpandEffect(True).SetDuration("500")
	cont.AddComponent(1,2,mostspentby.tostring)
	'
	mostspenton.Initialize(vue, "mostspenton", Me).SetIcon("insert_chart").SetFrom("0").SetTo("1600").SetText("Shopping")
	...
	'
	spentthisyear.Initialize(vue, "spentthisyear", Me).SetIcon("attach_money").SetFrom("0").SetTo("2000").SetText("Spent This Year")
