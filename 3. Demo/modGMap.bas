...
Sub Code
	...
	'
	Dim latitude As VMTextField = vm.CreateTextField("txtbuilding_latitude", Me).SetVmodel("building_latitude").SetLabel("GPS Latitude").SetInvalidMessage("The latitude is required!")
	frm.AddControl(latitude.TextField, latitude.ToString,1,2,0,0,0,0,12,6,6,6)
	'
	Dim longitude As VMTextField = vm.CreateTextField("txtbuilding_longitude", Me).SetVmodel("building_longitude").SetLabel("GPS Longitude").SetInvalidMessage("The address is required!")
